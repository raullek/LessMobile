...
data class AddBranchUserState(
    ...
    /** True when the cached current user is being edited AND is currently a merchant
     *  of this venue. A self-delete from merchant flow triggers a profile refetch
     *  and a navigation root switch back to partner. */
...
sealed interface AddBranchUserSideEffect {
    ...
    /** Self-deleted from a venue: the user is no longer a merchant — caller should
     *  switch the navigation root from MERCHANT back to PARTNER. */
