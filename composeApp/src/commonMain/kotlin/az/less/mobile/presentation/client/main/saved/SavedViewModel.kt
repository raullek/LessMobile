...
class SavedViewModel : ViewModel(), ContainerHost<SavedState, SavedSideEffect> {
    ...
    private fun handleItemClicked(itemId: String) =
        intent {
            ...
            val favoriteItem =
               state.favoriteItems.find { it.id == itemId }
            if (favoriteItem != null) {
               postSideEffect(
                    SavedSideEffect.NavigateToReserve(
                        favoriteItem
                    )
                )
            }
    ...
    private fun handleRemoveFromFavorites(itemId: String) =
        intent {
           reduce {
               state.copy(
    ...
    private fun handleExploreNewVenuesClicked() =
        intent {
