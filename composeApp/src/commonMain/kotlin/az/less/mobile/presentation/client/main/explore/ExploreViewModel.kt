...
import az.less.mobile.presentation.client.main.explore.models.MerchantOffer
...
class ExploreViewModel : ViewModel(), ContainerHost<ExploreState, ExploreSideEffect> {
    ...
    private fun handleFilterTypeSelected(filterType: FilterType) =
        intent {
            val newSelectedFilterType =
                if (state.selectedFilterType == filterType) null else filterType
           reduce {
               state.copy(
                    selectedFilterType = newSelectedFilterType,
                    filterItems = updateFilterItemsSelection(
                       state.filterItems,
                        "liked",
                        newSelectedFilterType == FilterType.LIKED
                    )
                )
            }
    ...
    private fun handleFilterItemClicked(filterId: String) =
        intent {
            when (filterId) {
                "filter_button" -> {
    ...
    private fun handleMapMarkerClicked(venueId: String) =
        intent {
           ...
           postSideEffect(
                ExploreSideEffect.NavigateToVenueDetail(
                    venueId
                )
            )
        }
    
    private fun handleLocationPermissionChanged(granted: Boolean) =
        intent {
           reduce {
               state.copy(
    ...
    private fun handleMapClick(latLong: LatLong) =
        intent {
           reduce {
    ...
    private fun handleVenueItemClicked(venueId: String) =
        intent {
           postSideEffect(
                ExploreSideEffect.NavigateToVenueDetail(
                    venueId
    ...
    private fun handleFilterSheetDismissed() =
        intent {
           reduce {
               state.copy(
    ...
    private fun handleFilterOptionClicked(categoryId: String, optionId: String) =
        intent {
