...
import az.less.mobile.presentation.partner.history.components.DownloadIconButton
import az.less.mobile.presentation.partner.history.components.FilterButton
import az.less.mobile.presentation.partner.history.components.IncomePositionItem
import az.less.mobile.presentation.partner.history.components.VenueSelectionBottomSheet
...
fun IncomeHistoryScreenContent(
    ...
    Column(
        ...
        Row(
            ...
            FilterButton(
                text = state.selectedVenue?.name ?: stringResource(Res.string.history_all_venues),
                onClick = { onIntent(IncomeHistoryIntent.OnVenueFilterClicked) },
                modifier = Modifier.weight(1f),
                isLoading = state.isLoadingVenues
            )
        }
    ...
    if (state.showVenuePicker) {
        VenueSelectionBottomSheet(
            venues = state.availableVenues,
            selectedVenue = state.selectedVenue,
            onSelect = { onIntent(IncomeHistoryIntent.OnVenueSelected(it)) },
            onClear = { onIntent(IncomeHistoryIntent.OnVenueCleared) },
            onDismiss = { onIntent(IncomeHistoryIntent.OnVenuePickerDismiss) }
        )
    }
}
...
private fun IncomeHistoryGroup(
    incomeHistory: az.less.mobile.presentation.partner.history.model.IncomeHistory,
