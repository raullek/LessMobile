...
import az.less.mobile.presentation.client.main.offers.models.FilterSegmentType
...
class CategoryOffersViewModel : ViewModel(), ContainerHost<CategoryOffersState, CategoryOffersSideEffect> {
    ...
    private fun handleFilterSegmentSelected(segmentId: String) =
      intent {
            val newSelectedId =
                if (state.selectedFilterSegmentId == segmentId) null else segmentId
          reduce {
              state.copy(
    ...
    private fun handleOfferItemClicked(offerId: String) =
    intent {
            val offerItem =
                state.offers.find { it.id == offerId }
            if (offerItem != null) {
                postSideEffect(
                    CategoryOffersSideEffect.NavigateToReserve(
                        offerItem
                    )
                )
            }
